...
class RoomMap:
    ...
    def from_ascii(cls, name: str, ascii_map: str):
        ...
        actors = {}
        loots = {}
        loot_counter, npc_counter = 1, 1

        for y, line in enumerate(lines):
            for x, char in enumerate(line.ljust(width)):
                if char == "M":
                    key = f"M{npc_counter}"
                    actors[key] = Actor(name=f"Monster {npc_counter}", symbol="M", pos=(x, y), facing="S")
                    npc_counter += 1
                    tiles[(x, y)] = Tile(" ", True, False, "Floor")
                elif char == "l":
                    key = f"l{loot_counter}"
                    loots[key] = Loot(name=f"Loot {loot_counter}", symbol="l", pos=(x, y))
                    loot_counter += 1
                    tiles[(x, y)] = Tile(" ", True, False, "Floor")
                else:
    ...
    def symbol_to_tile(symbol: str) -> Tile:
        mapping = {
            ...
            "V": Tile("V", True, False, "Vantage point"),
            "v": Tile("v", False, False, "Crack"),
            "G": Tile("G", True, False, "Gate"),
            "s": Tile("s", True, False, "Secret passage"),
    ...
    # -------------------------------------------
    def add_player(self, name: str, pos: Tuple[int, int], facing: str = "N"):
        self.actors["Player"] = Actor(name, "@", pos, facing)
    ...
    def describe_view_los(self, actor_name: str) -> str:
        ...
        for band in ("close", "mid", "far"):
            report_lines.append(f"{band.capitalize()}:")
            for sector in ("front","left", "right"):
                ...
                joined = ", ".join(parts)
                report_lines.append(f" {sector}, you see {joined}")
    ...
    # SAVE / LOAD
    # -------------------------------------------
    def save(self, path: str, fmt: str = "yaml"):
        data = {
            "name": self.name,
            "ascii_map": self.render_ascii(),
            #"width": self.width,
            #"height": self.height,
            #"tiles": {f"{x},{y}": t.to_dict() for (x, y), t in self.tiles.items()},
    ...
    @classmethod
    def load(cls, path: str, fmt: str = "yaml"):
        with open(path) as f:
